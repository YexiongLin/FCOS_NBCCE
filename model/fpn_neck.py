...
class FPN(nn.Module):
    ...
    def __init__(self,features=256,use_p5=True):
        ...
        '''
        self.merge_5 = nn.Conv2d(2*2048,2048, kernel_size=3, padding=1)
        self.merge_4 = nn.Conv2d(2*1024, 1024, kernel_size=3, padding=1)
        self.merge_3 = nn.Conv2d(2*512, 512, kernel_size=3, padding=1)
        '''
        self.merge_5 = nn.Conv2d(2*2048,2048, kernel_size=1)
        self.merge_4 = nn.Conv2d(2*1024, 1024, kernel_size=1)
        self.merge_3 = nn.Conv2d(2*512, 512, kernel_size=1)
    ...
    def forward(self,x):
        C3_0,C4_0,C5_0,C3_1,C4_1,C5_1=x
        #print(C3_0.shape,C3_1.shape)
        C3=torch.cat((C3_0,C3_1),1)
        #print(C3.shape)
        C3=self.merge_3(C3)
        C4=torch.cat((C4_0,C4_1),1)
        C4=self.merge_4(C4)
        C5=torch.cat((C5_0,C5_1),1)
        C5=self.merge_5(C5)
