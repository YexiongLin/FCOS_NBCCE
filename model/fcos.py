...
class FCOS(nn.Module):
    ...
    def __init__(self,config=None):
        ...
        self.backbone0=resnet50(pretrained=config.pretrained,in_channels=3,if_include_top=False)
        self.backbone1=resnet50(pretrained=config.pretrained,in_channels=1,if_include_top=False)
    ...
    def train(self,mode=True):
        ...
        if self.config.freeze_stage_1:
            self.backbone0.freeze_stages(1)
            self.backbone1.freeze_stages(1)
    ...
    def forward(self,x0,x1):
        ...
        '''
        C3_0,C4_0,C5_0=self.backbone0(x0)
        C3_1,C4_1,C5_1=self.backbone1(x1)
        #print(C3_0.shape,C4_0.shape,C5_0.shape)
        all_P=self.fpn([C3_0,C4_0,C5_0,C3_1,C4_1,C5_1])
...
class DetectHead(nn.Module):
    ...
    def forward(self,inputs):
        ...
        cls_scores,_=torch.max(cls_preds,dim=-1)#[batch_size,sum(_h*_w)]
        cls_scores=cls_preds[:,:,0]
        #print(cls_scores.shape)
        cls_preds=torch.tensor(cls_preds[:,:,1:]>self.config.theta)
        #print(cls_preds.shape)
        cls_classes=bin2int(cls_preds).to(cls_scores.device)
    ...
    def batched_nms(self,boxes, scores, idxs, iou_threshold):
        ...
        keep = self.box_nms(boxes, scores, iou_threshold)
        return keep
...
class FCOSDetector(nn.Module):
    ...
    def forward(self,inputs):
        ...
        if self.mode=="training":
            batch_imgs_rgb,batch_imgs_d,batch_boxes,batch_classes=inputs
            #print(batch_imgs.shape)
            out=self.fcos_body(batch_imgs_rgb,batch_imgs_d)
            '''
            print(out[2][0].max())
            print(out[2][1].max())
            print(out[2][2].max())
            ...
            '''
            batch_imgs_rgb,batch_imgs_d=inputs
            out=self.fcos_body(batch_imgs_rgb,batch_imgs_d)
            scores,classes,boxes=self.detection_head(out)
            boxes=self.clip_boxes(batch_imgs_rgb,boxes)
            return scores,classes,boxes
