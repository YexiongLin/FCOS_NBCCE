...
class GenTargets(nn.Module):
    ...
    def _gen_level_targets(self,out,gt_boxes,classes,stride,limit_range,sample_radiu_ratio=1.5):
        ...
        volume_max=torch.max(classes,dim=-1)[0]#[batch_size,h*w,m]

        mask_in_gtboxes=off_min>0
        # mask_in_level=(off_max>limit_range[0])&(off_max<=limit_range[1]) # level using bounding box in original FCOS
        mask_in_level=((volume_max>limit_range[0])&(volume_max<=limit_range[1])).reshape(-1,1,1)
...
def convert_bin(x):
    result=[]
    for i in x:
        result.append(tobin(i[0].cpu().numpy()))
...
def NBCCE_loss(preds,targets):
    ...
    '''
    #print(preds.shape)
    L=torch.zeros(preds.shape[1]).to(device=preds.device)
    for i in range(preds.shape[1]):
        loss=(2**(preds.shape[1]-i))*(torch.mul(-targets[:,i], torch.log(preds[:,i]))-torch.mul((1-targets[:,i]),torch.log(1-preds[:,i])))
        L[i]=torch.mean(loss)
    return torch.mean(L)
...
def compute_cls_loss(preds,targets,mask):
    ...
    for batch_index in range(batch_size):
        pred_pos=preds[batch_index][:,0:1]#[sum(_h*_w),class_num]
        target_pos=targets[batch_index]#[sum(_h*_w),1]
        target_pos_tmp=convert_bin(target_pos).to(pred_pos.device)#sparse-->binary
        target_pos=target_pos_tmp[:,0:1]
        loss.append(focal_loss_from_logits(pred_pos,target_pos).view(1))
    return torch.cat(loss,dim=0)/num_pos#[batch_size,]

def compute_NBCCE_loss(preds,targets,mask):
    '''
    Args  
    preds: list contains five level pred [batch_size,class_num,_h,_w]
    targets: [batch_size,sum(_h*_w),1]
    ...
    loss=[]
    for batch_index in range(batch_size):
        pred_pos=preds[batch_index][:,1:]#[sum(_h*_w),class_num]
        target_pos=targets[batch_index]#[sum(_h*_w),1]
        target_pos_tmp=convert_bin(target_pos).to(pred_pos.device)#sparse-->binary
        target_pos=target_pos_tmp[:,1:]
        loss.append(50.*NBCCE_loss(pred_pos,target_pos).view(1))
    return torch.cat(loss,dim=0)/num_pos#[batch_size,]
...
class LOSS(nn.Module):
    ...
    def forward(self,inputs):
        ...
        nbcce_loss=compute_NBCCE_loss(cls_logits,cls_targets,mask_pos).mean()
        ...
        if self.config.add_centerness:
            total_loss=cls_loss+nbcce_loss+cnt_loss+reg_loss
            return cls_loss,nbcce_loss,cnt_loss,reg_loss,total_loss
        else:
