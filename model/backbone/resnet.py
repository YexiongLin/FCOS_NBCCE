...
class ResNet(nn.Module):
    ...
    def __init__(self, block, layers, in_channels=3, num_classes=1000,if_include_top=False):
        self.inplanes = 64
...
def resnet50(pretrained=False, in_channels=3, **kwargs):
    ...
    if pretrained:
        if in_channels==1:
            pretext_model=torch.load('./resnet50.pth')
            model_dict=model.state_dict()
            state_dict={k:v for k,v in pretext_model.items() if (k in model_dict.keys())and (k not in ['conv1.weight'])}
