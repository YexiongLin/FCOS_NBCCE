    ...
    for label in range(num_cls)[1:]:
        ...
        for sample_gts, sample_pred_box, sample_scores in zip(gt_single_cls, bbox_single_cls, scores_single_cls):
            ...
            for index in range(len(sample_pred_box)):
                ...
                if max_overlap.any() >= iou_thread and gt_for_box not in assigned_gt:
                    fp = np.append(fp, 0)
                    tp = np.append(tp, 1)
...
if __name__=="__main__":
    ...
    parser.add_argument('--model', type=int, help='model number', default=38)
    args = parser.parse_args()

    from model.fcos import FCOSDetector
    #from demo import convertSyncBNtoBN
    
    test_datas, test_labels=pd.read_csv('testsets.csv').iloc[:,:].values.T
    test_datas, test_labels=get_files(test_datas, test_labels)
    test_dataset = liquidDataset(test_datas,test_labels)
    ...
    print("INFO===>test dataset has %d imgs"%len(test_dataset))
    ...
    model.load_state_dict(torch.load("./checkpoint/model_%d.pth"%(args.model),map_location=torch.device('cpu')))
    ...
    iou=0.0   
    recall=0
    for img, depth,boxes,classes in eval_loader:
        with torch.no_grad():
            out=model([img.cuda(),depth.cuda()])
        ...
        # print(out[2][0],out[1][0],out[0][0])
        # print(out[1][0],classes[0])
        
        if(iou_2d(boxes[0].cpu(), out[2][0].cpu()).shape[1]!=0):
            error+=((torch.abs(out[1][0].cpu()-classes[0])/classes[0]).mean())
            iou+=iou_2d(boxes[0].cpu(), out[2][0].cpu())[0][0]
            num+=1
            print(num,end='\r')
    ...
    #all_AP=eval_ap_2d(gt_boxes,gt_classes,pred_boxes,pred_classes,pred_scores,0.5,1024)
    '''
    ...
    for i in pred_classes:
        ...
        for j in range(i.shape[0]):
            tmp.append(1)
        pred_classes_out.append(tmp)
    '''
    
    all_AP=eval_ap_2d(gt_boxes,[1 for i in gt_classes],pred_boxes,[1 for i in gt_classes],pred_scores,0.5,2)
    print("all classes AP=====>\n")
    for key,value in all_AP.items():
        print('ap for {} is {}'.format(int(key),value))
    mAP=0.
    for class_id,class_mAP in all_AP.items():
        mAP+=float(class_mAP)
    #mAP/=(len(test_dataset.CLASSES_NAME)-1)
    print("mAP=====>%.3f\n"%mAP)
